...
export default function AdminDashboard() {
  ...
  const initialFormState = {
    ...
    // ── Product Type ──────────────────────────────────────────────────────────
    productType: "ready" as "ready" | "customized",   // "ready" | "customized"
    readyMadeCode: "",        // 12-digit barcode / product code (Ready Made only)
    materialPrice: "",        // max 7 digits  (Customized only)
    designPrice: "",          // max 5 digits  (Customized only)
  };
...
const payload = {
  ...
  designPrice:   formData.productType === "customized" ? (Number(formData.designPrice) || 0) : 0,
            ...
            <div className="flex-1 p-12 overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-4">Product Type</label>
                  <div className="flex gap-3 mb-5">
                    {/* Ready Made */}
                    <button
                      type="button"
                      onClick={() => setFormData({ ...formData, productType: "ready", materialPrice: "", designPrice: "" })}
                      className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border-2 flex items-center justify-center gap-2 ${
                    ...
                    >
                      <PlusCircle size={14} /> Customized
                    </button>
                  </div>

                  {/* Ready Made → 12-digit code */}
                  {formData.productType === "ready" && (
                    <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">
                        Product Code <span className="text-gray-300 normal-case font-semibold">(12 digits)</span>
                      </label>
                      <input
                        type="text"
                        inputMode="numeric"
                        ...
                        className="w-full p-4 bg-white rounded-2xl outline-none font-black text-gray-800 border border-gray-200 focus:border-[#c12a52] transition-all tracking-[0.2em] text-lg"
                      />
                      <p className="text-[10px] text-gray-300 font-bold mt-1">{formData.readyMadeCode.length}/12 digits</p>
                    </div>
                  )}

                  {/* Customized → Material Price + Design Price */}
                  {formData.productType === "customized" && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase block mb-2">
                          Material Price <span className="text-gray-300 normal-case font-semibold">(max 7 digits)</span>
                        </label>
                        <div className="relative">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-black text-gray-400">LKR</span>
                          <input
                            type="number"
                            ...
                            placeholder="0000000"
                            className="w-full pl-14 pr-4 py-4 bg-white rounded-2xl outline-none font-black text-blue-600 border border-gray-200 focus:border-blue-400 transition-all"
                          />
                        </div>
                        <p className="text-[10px] text-gray-300 font-bold mt-1">Max: 9,999,999</p>
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase block mb-2">
                          Design Price <span className="text-gray-300 normal-case font-semibold">(max 5 digits)</span>
                        </label>
                        <div className="relative">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-black text-gray-400">LKR</span>
                          <input
                            ...
                            onChange={e => {
                              ...
                              setFormData({ ...formData, designPrice: val });
                            }}
                            placeholder="00000"
                            className="w-full pl-14 pr-4 py-4 bg-white rounded-2xl outline-none font-black text-purple-600 border border-gray-200 focus:border-purple-400 transition-all"
                          />
                        </div>
                        <p className="text-[10px] text-gray-300 font-bold mt-1">Max: 99,999</p>
                      </div>
                    </div>
                  )}
                </div>
